...
- **По умолчанию:** Автоматически определяется из текущего репозитория в HTTPS формате
- **Примечание:** 
...
- **Примечание:** 
  - Переменная передается напрямую в docker-compose как переменная окружения
  - Не требуется создавать `.env` файл на сервере
  - Токен хранится безопасно в GitHub Secrets и передается только во время деплоя
